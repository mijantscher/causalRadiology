...
def main(args):
    ...
    for p_grp in p_grp_folders:
        ...
        for p in tqdm(p_folders):
            ...
            for s in studies:
                ...
                study_sectioned = [s_stem[1:]]
                for sn in ('indication', 'history', 'comparison', 'notification'):
                    if sn in section_names:
                        idx = list_rindex(section_names, sn)
                        # remove all linebreaks from string
                        study_sectioned.append(
                            re.sub(' +', ' ', sections[idx].replace("\n", " ").replace("\r", " ").strip()))
                    else:
    ...
    if len(patient_studies) > 0:
        ...
        with open(output_path / 'mimic_cxr_sectioned.csv', 'w') as fp:
            ...
            # write header
            csvwriter.writerow(['study', 'indication', 'history', 'comparison', 'notification'])
